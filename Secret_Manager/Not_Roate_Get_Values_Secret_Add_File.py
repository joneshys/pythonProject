...
## Revome test.txt for new Values secrets
os.remove("test.txt")
...
for count in range(len(clientes)):
    ...
    # print(SecretLists[0]) # Punto de control

    ## Rotation passwd RDS
    #RotationLambdaARN = SecretLists[0]['RotationLambdaARN']
    # print(RotationLambdaARN) # Punto de control
    ...
    #    SecretString_Name = SecretString['Name']
        # print(SecretString)# Punto de control
    #    rotate_secret = Secret.rotate_secret(SecretId=Name, RotationLambdaARN=RotationLambdaARN,RotationRules={'AutomaticallyAfterDays': 25})

    ## Get value secret passwd EDS
    AccountAlias = IAMClient.list_account_aliases()['AccountAliases'][0].upper()
    # print(AccountAlias)# Punto de control
    Get_Secret_Values = []
...
session.quit()
print('Mail Sent')# Punto de control